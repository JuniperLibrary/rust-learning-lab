...
> 这些是看代码最常遇到、但不专门学就是不知道的知识点。建议先过一遍，再进入阶段一。

| 知识点 | 难度 | 预估时间 | 说明 |
|--------|------|----------|------|
| 基础数据类型 | ★ | 2h | 整数（i32/u32/usize/isize 等）、浮点 f32/f64、bool、char、类型大小与范围 |
| 复合类型 | ★ | 1.5h | 元组 Tuple、数组 Array、切片 Slice `&[T]` |
| 控制流实战 | ★ | 1.5h | `loop` 返回值、`while`、`for` 循环、`break`/`continue`、标签循环 |
| 字符串深入 | ★★ | 2h | `&str` vs `String` 的完整理解：编码、切片、常用方法（trim/split/parse/replace/chars） |
| 类型转换 | ★★ | 1.5h | `as` 关键字、`From`/`Into` trait、`TryFrom`/`TryInto` |
| 常用集合 | ★★ | 2h | `HashMap<K, V>`、`HashSet<T>`、Vec 常用方法详解 |

**预期效果：** 再看代码时，遇到 `Vec::len()` 返回 `usize`、看到 `.parse::<i32>()`、遇到 `as` 转换不会懵。

```rust
// 阶段零学完后能看懂这些：
let n: usize = vec![1, 2, 3].len();       // usize 类型
let sum = 5u32 + 10;                       // 字面量后缀
let nums: [i32; 3] = [1, 2, 3];           // 数组 vs Vec
let s: &str = &s[0..5];                    // 字符串切片
let n: i32 = "42".parse().unwrap();        // parse 类型推断
let map: HashMap<&str, i32> = [("a", 1)].into_iter().collect();
```
